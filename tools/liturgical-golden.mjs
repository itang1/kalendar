...
// One compact line per day: YYYY-MM-DD|season|color|solemnity|week|feastName|title
function lineFor(engine, date) {
  ...
  const title = engine.liturgicalDayTitle(info, date) || '';
  return `${y}-${m}-${d}|${info.season}|${info.color.name}|${info.isSolemnity ? 1 : 0}|${week}|${feast}|${title}`;
}
